...
8.5 光盘写入工具：
   ...
   -graft-point isodir=systemdir：有转嫁或移嫁的意思。其实，不加这个，在镜像
    ...
    8.5.2 制作、修改可启动光盘镜像文件系统：
    使用isoinfo来查看是否为我们想要的光盘系统；
    挂载iso文件到/mnt；生成新的目录，用rsync将数据复制到新生成的目录里面。
    rsync可以完整的复制所有的权限属性等数据，也能够进行镜像处理。之后用
    mkisofs就可以制作出来了。


    8.5.3cdrecord：光盘刻录工具。
    现在使用wodim，它可以兼容cdrecord。（暂时用不到，就不看了）



8.6其他常见的压缩与备份工具：
   8.6.1 dd
   不仅可以用来作为制作文件的工具，还有备份的功能，制作usb启动盘的功能。
   当然，由于dd是通过磁盘的使用扇区的一个个读取，同时写入到备份文件，因此
   该备份文件会将一些没有用到的扇区也写入，因此很笨拙，不如xfsdump好用。


   8.6.2 cpio
   cpio可以备份任何东西，但是cpio不会主动的去找文件来备份。
   cpio -ovcB > [find|device] 备份
   cpio -ivcdu < [find|device] 还原
   cpio -ivct < [find|device] 查看
